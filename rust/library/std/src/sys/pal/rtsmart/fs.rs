...
impl FileAttr {
    #[cfg(all(not(target_os = "vxworks"), not(target_os = "rtsmart")))]
    pub fn modified(&self) -> io::Result<SystemTime> {
        Ok(SystemTime::from(libc::timespec {
            tv_sec: self.stat.st_mtime as libc::time_t,
    ...
    #[cfg(any(target_os = "vxworks", target_os = "rtsmart"))]
    pub fn modified(&self) -> io::Result<SystemTime> {
        Ok(SystemTime::from(libc::timespec {
            tv_sec: self.stat.st_mtime as libc::time_t,
            tv_nsec: 0,
...
impl fmt::Debug for File {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        #[cfg(not(any(target_os = "linux", target_os = "macos", target_os = "vxworks", target_os = "rtsmart")))]
        fn get_path(_fd: c_int) -> Option<PathBuf> {
            // FIXME(#24570): implement this for other Unix platforms
            None
        }

        #[cfg(any(target_os = "linux", target_os = "macos", target_os = "vxworks", target_os = "rtsmart"))]
...
pub fn link(original: &Path, link: &Path) -> io::Result<()> {
    ...
    let link = cstr(link)?;
    cfg_if::cfg_if! {
        if #[cfg(any(target_os = "vxworks", target_os = "redox", target_os = "android", target_os = "rtsmart"))] {
