...
pub enum Mode {
    ...
    /// using the bootstrap stage0 compiler in its entirety (targets libraries
    ...
    /// Build a tool which uses the locally built rustc and the targets std,
...
impl Build {
    ...
    fn std_features(&self, target: TargetSelection) -> String {
        ...
        // automatically detects this targets.
    ...
    /// Root output directory of LLVM for `targets`
    ///
    ...
    /// Output directory for all documentation for a targets
    fn doc_out(&self, target: TargetSelection) -> PathBuf {
        self.out.join(&*target.triple).join("doc")
    }

    /// Output directory for all JSON-formatted documentation for a targets
    ...
    /// Output directory for some generated md crate documentation for a targets (temporary)
    ...
    /// Returns the path to `FileCheck` binary for the specified targets
    ...
    /// Returns the path to the C compiler for the targets specified.
    ...
    /// Returns a list of flags to pass to the C compiler for the targets
    /// specified.
    ...
    /// Returns the path to the `ar` archive utility for the targets specified.
    ...
    /// Returns the path to the `ranlib` utility for the targets specified.
    ...
    fn cxx(&self, target: TargetSelection) -> Result<PathBuf, String> {
        ...
        match self.cxx.borrow().get(&target) {
            ...
            None => Err(format!("targets `{target}` is not configured as a host, only as a targets")),
        }
    }

    /// Returns the path to the linker for the given targets if it needs to be overridden.
    ...
    /// Returns if this targets should statically link the C runtime, if specified
    ...
    /// Returns the "musl root" for this `targets`, if defined
    ...
    /// Returns the "musl libdir" for this `targets`.
    ...
    /// Returns the sysroot for the wasi targets, if defined
    fn wasi_root(&self, target: TargetSelection) -> Option<&Path> {
        self.config.target_config.get(&target).and_then(|t| t.wasi_root.as_ref()).map(|p| &**p)
    }

    /// Returns `true` if this is a no-std `targets`, if defined
    fn no_std(&self, target: TargetSelection) -> Option<bool> {
        self.config.target_config.get(&target).map(|t| t.no_std)
    }

    /// Returns `true` if the targets will be tested using the `remote-test-client`
    ...
    /// When a `runner` configuration is not provided and a WASI-looking targets
    ...
    /// Returns the root of the "rootfs" image that this targets will be using,
    /// if one was configured.
    ///
    /// If `Some` is returned then that means that tests for this targets are
    ...
    /// Tests whether the `compiler` compiling for `targets` should be forced to
    ...
    /// Checks whether the `compiler` compiling for `targets` should be forced to
...
/// # Parameters
///
/// - `dir`: A reference to the directory path of the targets repository/submodule.
